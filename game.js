/* ============================================================
   Idle Civ — prototype game logic (v3)
...
// Buildings. `base` = cost of the first; each next costs *scale more.
...
// Minimal line-art doodles for the Settlement panel -- no map, just icons.
...
function freshState() {
  return {
    ...
    builds:{ hut: 0, woodshed: 0, granary: 0, dryingRack: 0, lumberCamp: 0, stonePit: 0, infirmary: 0 },
...
function idle() { return S.pop - jobsUsed(); }
...
// Production (per second) plus the food upkeep line.
function rates() {
  const m = mults();
  const prod = {
...
// How many of this building are already owned or waiting in the queue --
// keeps the escalating price honest even when you queue several at once.
function pendingCount(id) { return S.buildQueue.filter((q) => q.id === id).length; }

function buildCost(def) {
  if (def.kind === "upgrade") return { ...def.base };  // flat, never scales, never repeats
...
function defById(id) { return BUILDINGS.find((b) => b.id === id) || UPGRADES.find((u) => u.id === id); }

// Once a building/upgrade's reveal() condition has been true, it stays visible
// forever -- otherwise a threshold-based reveal (e.g. "wood >= 8") could flicker
// the whole panel back into hiding the moment that resource dips, which is
// exactly the kind of un-reveal nothing else in this game ever does.
...
// ---------- Events ---------------------------------------------
// The generic occurrence engine. Every entry is one of two shapes:
...
// picked at random). This is the whole contract -- adding a new event (conflict,
// windfalls, rare disasters) never touches the engine below, only this list.
const EVENTS = [
  {
...
// A settler dies: population drops, and if that leaves more workers assigned
// than people alive, the excess is pulled back to idle (wood/stone before food,
// so a starving settlement's last forager is the last to go).
function removeSettler() {
  if (S.pop <= 1) return;
  S.pop -= 1;
  let over = jobsUsed() - S.pop;
...
function step(dt) {
  ...
  if (S.res.food <= 0 && r.foodNet < 0) {
    S.res.food = 0;
    if (SIM) { SIM_STOP = true; return; }  // offline never kills you, just halts
    die();
    return;
  }
  ...
  // Population growth, sickness, and anything else on the EVENTS list.
  resolveEvents(dt);
}

function die() {
  S.dead = true;
  log("The last of your people starve. The settlement falls silent.", "big");
...
// Clicking a building or upgrade pays for it immediately and drops it in the
// (shared) queue. If the queue was empty it starts progressing right away;
// otherwise it waits its turn behind whatever's already building.
function build(def) {
  if (S.dead) return;
...
// for a full refund of what was actually paid for it.
...
function renderPeople() {
  document.getElementById("popTotal").textContent = S.pop;
...
function renderQueue() {
  ...
  S.buildQueue.forEach((item, i) => {
    ...
    card.querySelector(".q-label").textContent = i === 0 ? `Raising: ${def.name}` : `Queued: ${def.name}`;
...
function renderHoldings() {
  ...
  for (const def of owned) {
    let tile = document.getElementById("hold-" + def.id);
    if (!tile) {
      tile = document.createElement("div");
      tile.className = "holding";
      tile.id = "hold-" + def.id;
      tile.innerHTML =
        `<span class="h-icon">${BUILDING_ICONS[def.id] || ""}</span>` +
        `<span class="h-name">${def.name}</span>` +
        `<span class="h-count" id="hcount-${def.id}"></span>`;
      body.appendChild(tile);
    }
    document.getElementById("hcount-" + def.id).textContent = S.builds[def.id];
  }
}
...
function renderBuildings() {
  ...
  for (const def of BUILDINGS) {
    ...
    const cost = buildCost(def);
    const costStr = Object.keys(cost)
      .map((k) => {
    ...
    card.innerHTML =
      ...
      `<div class="b-cost">${costStr}<span class="b-time">${def.buildTime}s build</span></div>`;
    card.disabled = S.dead || !canAfford(cost);
  }
...
function simulateOffline() {
  ...
  else if (g.pop < 0) parts.push(`${-g.pop} settler${g.pop < -1 ? "s" : ""} lost to illness`);
  const mins = Math.floor(capped / 60);
  if (SIM_STOP) log(`You return to find the stores emptied — your people barely hung on.`, "bad");
  else if (parts.length) log(`While you were away (${mins}m): ${parts.join(", ")}.`, "good");
}
