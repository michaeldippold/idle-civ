// Instanced props (guide §8), built from stylized primitives.
//
// Asset note: the plan's first choice was 2-4 CC0 low-poly GLBs (Quaternius /
// Kenney). The Quaternius GitHub repo probed 404 and no reliable raw GLB URL
// surfaced, so per the fallback clause these are primitive props — cone+cyl
// trees, box+pyramid huts, icosahedron rocks — with the HDRI+ACES+AO stack
// doing the visual work. Placement, rotation and ±10% scale jitter are all
// deterministic (hashed from tile id; no Math.random).

import * as THREE from "three";
import { axialToWorld, hash01 } from "./hex.js";
...
class Part {
  ...
  add(x, y, z, rotY, scale, color) {
    this.items.push({ x, y, z, rotY, scale, color });
  }
  build() {
...
export function buildProps(world, elev) {
  ...
  const hut = (id, x, y, z, tag, scale) => {
    ...
    hutWall.add(x, y + 0.08 * s, z, rot, s, null);
  ...
  for (const id in world.places) {
    ...
    if (p.terrain === "forest") {
      ...
      for (let i = 0; i < n; i++) {
        ...
        trunk.add(cx + dx, y + 0.11 * s, cz + dz, rot, s, null);
        canopy.add(cx + dx, y + (0.2 + 0.26) * s, cz + dz, rot, s, canopyCol.clone());
      }
    }
    ...
    if (p.terrain === "hills") {
      ...
      for (let i = 0; i < n; i++) {
        ...
        rock.add(cx + dx, y + 0.07 * s, cz + dz,
          ...
          { x: s, y: s * (0.6 + 0.5 * hash01(id + ":ry" + i)), z: s }, null);
      }
    }
    ...
    if (id === world.home) {
      ...
      for (let i = 0; i < 3; i++) {
        ...
        tower.add(cx + dx, y + 0.22 * s, cz + dz, rot, s, null);
        towerRoof.add(cx + dx, y + (0.44 + 0.1) * s, cz + dz, rot + Math.PI / 4, s, roofCol.clone());
      }
    } else if (p.minor) {
